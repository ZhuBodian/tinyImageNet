...
def calculate_split_info(path: str, label_dict: dict, rate: float = 0.2):
    ...
    for train_index, test_index in split.split(data, data["label"]):
        ...
        strat_test_set = data.iloc[test_index, :]  # 保证测试集
        strat_train_set.to_csv(os.path.join(path, "new_train.csv"))
        strat_test_set.to_csv(os.path.join(path, "new_test.csv"))
